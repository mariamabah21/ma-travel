...
interface FormInput {
  files: DocumentToUpload[]; // ✅ consistent type
}
...
export default function FilesForm(props: Props) {
  const {
    ...
    removingFilePath,
    uploadErrors,
  } = useFilesUploadForm(props); // ✅ uses fixed initialization
        ...
        return (
          ...
          >
            {showCard && (
              <>
                ...
                {isPhotosForm && (
                  ...
                  >
                    {/* ✅ FIXED: use file.url not file.src */}
                    <PhotoCard
                      src={file.url}
                      onRemoveClick={() => onFileRemove(index)}
                      uploadProgress={uploadProgresses[index]}
            ...
            <Controller
              name={`files.${index}`}
              control={control}
              // ✅ Changed validation message to match files
...
function useFilesUploadForm(props: Props) {
  ...
  // ✅ Map defaultFiles (TripFile[]) to DocumentToUpload[]
  const { watch, handleSubmit, control } = useForm<FormInput>({
    defaultValues: {
      files: props.defaultFiles.map((file) => ({
